...
async function sendMkinTokens(recipientWalletAddress, amount) {
  ...
  if (!mkinTokenMint || !gatekeeperKeypairJson) {
    throw new Error(
      "Missing MKIN_TOKEN_MINT or GATEKEEPER_KEYPAIR environment variables",
    );
  }

  console.log(
    "[MKIN Transfer] Sending " + amount + " MKIN to " + recipientWalletAddress,
  );
  ...
  const toTokenAccount = await getAssociatedTokenAddress(
    mkinMint,
    recipientPubkey,
    true, // allowOwnerOffCurve (Support PDAs/Multisigs)
  );
  ...
  // Add priority fee to ensure inclusion (prevention against drops)
  // 50,000 microLamports is a reasonable baseline for prioritizing over spam
  transaction.add(
  ...
  try {
    ...
    console.log(
      "[MKIN Transfer] Recipient token account exists with balance: " +
        parseInt(accountInfo.amount) / Math.pow(10, 9),
    );
  } catch (error) {
    recipientAccountExists = false;
    console.log(
      "[MKIN Transfer] Recipient token account does not exist - will create it",
    );
    console.log(
      "[MKIN Transfer] Account creation will cost ~0.00203928 SOL (paid by hot wallet)",
    );
  ...
  try {
    ...
    const availableBalance =
      Number(senderAccount.amount) / Math.pow(10, decimals);
    ...
    console.error(
      `[MKIN Transfer] Failed to check hot wallet balance: ${balanceError.message}`,
    );
  ...
  try {
    ...
    console.log(
      `[MKIN Transfer] Hot wallet SOL balance: ${solBalanceInSol.toFixed(6)} SOL`,
    );
    ...
    console.error(
      `[MKIN Transfer] Failed to check SOL balance: ${solBalanceError.message}`,
    );
  }
  ...
  console.log(
    "[MKIN Transfer] Last valid block height: " + lastValidBlockHeight,
  );
...
async function getHotWalletBalance() {
  ...
  const availableMkin =
    parseInt(balance.value.amount) / Math.pow(10, balance.value.decimals);
